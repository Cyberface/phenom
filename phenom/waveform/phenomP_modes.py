...
from numpy import exp, arange, zeros, array, conj, sin, cos, dot, max, arccos, arctan2, unwrap, angle, append
...
class PhenomP_modes(object):
    ...
    def __init__(self,
                ...
                f_min=20., f_max=0., delta_f=1/64.,
                distance=1e6 * Constants.PC_SI,
                fRef=0., phiRef=0., inclination=0.,
                modes=False,jaligned=True):
        """
        input:
        m1 (Msun)
        m2 (Msun)
        ...
        mode_dict = {}
        for m in [-2,-1,0,1,2]:
            mode_dict.update({"{l},{m}".format(l=2, m=m): array([])} )

        for i in range(len(omega_flist_hz)):
            # omega = omega_flist_hz[i] * self.p['M_sec']
            mode, self.alpha[i], self.epsilon[i], self.beta[i] = self.do_the_twist_one_frequency_modes(self.p, i, omega_flist_hz)
            for md in mode_dict.keys():
                if i == 0:
                    mode_dict[md] = array([mode[md]])
                else:
                    mode_dict[md] = append(mode_dict[md],mode[md])


        self.mode_dictionary = mode_dict
    ...
    def do_the_twist_one_frequency_modes(self, p, i, omega_flist_hz):
        ...
        eps_phase_hP = exp(-2*1.j*epsilon) * self.hP[i]
        ...
        cexp_im_alpha = array([cexp_m2i_alpha, cexp_mi_alpha, 1.0, cexp_i_alpha, cexp_2i_alpha]) * eps_phase_hP # add phase here

        hlm = {}
        for m in [-2, -1, 0, 1, 2]:
            hlm.update({"{l},{m}".format(l=2, m=m): dm2[m+2] * cexp_im_alpha[2-m] })

        return hlm, alpha, epsilon, arccos(cBetah) * 2.
